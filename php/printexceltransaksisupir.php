...
<body>
	...
	<table border="1">
		...
		$data = mysqli_query($koneksi,"SELECT user.nama AS usernama, user.telp AS usertelp, transaksi.tanggal, transaksi.idtransaksi, transaksi.tempat_awal, transaksi.tujuan, transaksi.plat_nomor, transaksi.no_supir, transaksi.id_user, mobil.merek, supir.nama AS supirnama, supir.telp AS supirtelp, status.status
        FROM transaksi 
        ...
        WHERE transaksi.no_supir='$idSupir' AND year(tanggal)='$Tahun' AND month(tanggal)='$Bulan'");
		$no = 1;

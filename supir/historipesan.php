...
<body>
    ...
    <div class="container">
      ...
      <nav class="nav-menu float-right d-none d-lg-block">
        <ul>
          ...
          <li class="active"><a href="#">Histori Pesan</a></li>
  ...
  <main id="main">
    ...
    <!-- ======= Pencarian Data Transaksi Berdasarkan Tahun dan Bulan ======= -->
    <div class="row">
      ...
      <?php
        ...
        $total_pages_sql = "SELECT COUNT(*) FROM transaksi WHERE no_supir='$idSupir'";
        $result = mysqli_query($conn,$total_pages_sql);
        $total_rows = mysqli_fetch_array($result)[0];
        $total_pages = ceil($total_rows / $no_of_records_per_page);

        $sql = "SELECT user.nama AS usernama, 
        user.telp AS usertelp, 
        transaksi.idtransaksi, 
        transaksi.tanggal, 
        transaksi.tempat_awal, 
        transaksi.tujuan, 
        transaksi.plat_nomor, 
        transaksi.no_supir AS nosup, 
        mobil.merek, 
        supir.nama AS supirnama, 
        supir.telp AS supirtelp, 
        status.status
        FROM transaksi 
        ...
        WHERE transaksi.no_supir='$idSupir' AND year(tanggal)='$Tahun' AND month(tanggal)='$Bulan' AND transaksi.status>='1'
        ...
        while($row = mysqli_fetch_array($res_data)){
            ...
            echo "<td><a href='datapesanan1.php?idtransaksi=".$row['idtransaksi']."'>Lihat</a></td>";
            echo "</tr>";
          }
        mysqli_close($conn);
      }else{
      ...
      <?php
        ...
        INNER JOIN user ON transaksi.id_user=user.no_karyawan
        INNER JOIN status ON transaksi.status=status.id
        WHERE transaksi.no_supir='$idSupir' AND transaksi.status>='1' 
        ORDER BY tanggal DESC
        LIMIT $offset, $no_of_records_per_page";
        $res_data = mysqli_query($conn,$sql);
        while($row = mysqli_fetch_array($res_data)){
